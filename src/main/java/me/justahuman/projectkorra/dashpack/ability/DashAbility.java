...
public interface DashAbility extends PlayerLocationAbility, PassiveAbility, MyAddonAbility {
    ...
    @Override
    default boolean isHiddenAbility() {
        return false;
    }
    ...
    default boolean maxAirApplicable(Block block) {
    ...
    default <A extends CoreAbility & DashAbility> boolean canDash(A ability) {
        ...
        if (!player.isOnGround() && !player.isInWaterOrBubbleColumn()) {
            if (!ability.inAir()) {
                ...
                if (!maxAirApplicable(location.getBlock())) {
                    return false;
                }
            }
        }
    ...
    default void addUsage(Player player) {
        Utils.addComboAbility(player, new DashInformation(getName(), System.currentTimeMillis(), getDirection().describing(this, player)));
    }
    ...
    default int pushParticleCount() {
        return DashPack.instance.getConfig().getInt("Dash.PushParticles");
    }
    ...
    default Vector getVelocityDirection() {
        return getVelocityDirection(getPlayer());
    }

    default Vector getVelocityDirection(Player player) {
        return getDirection().getVector(player, getInput(), getPitchRestriction());
    }
